...
Objetivo: >= 85% success rate

Este es el entorno más difícil - requiere GENERALIZACIÓN.
Usa Transfer Learning desde el agente del entorno 2.
...
def load_pretrained_weights_full(agent, pretrained_path, verbose=True):
    """
    Carga pesos preentrenados del entorno 2 (misma arquitectura y acciones).
    Transferencia completa ya que tienen la misma estructura.
    """
    if verbose:
        print("\n🔄 TRANSFER LEARNING desde Entorno 2")
    
    try:
        checkpoint = torch.load(pretrained_path, map_location=agent.device, weights_only=False)
        
        if 'policy_net_state_dict' in checkpoint:
            agent.policy_net.load_state_dict(checkpoint['policy_net_state_dict'])
            agent.target_net.load_state_dict(checkpoint['target_net_state_dict'])
            if verbose:
                print(f"   ✅ Cargado desde checkpoint (formato completo)")
                if 'best_success_rate' in checkpoint:
                    print(f"      - Success rate original: {checkpoint['best_success_rate']:.1f}%")
        else:
            agent.policy_net.load_state_dict(checkpoint)
            agent.target_net.load_state_dict(checkpoint)
            if verbose:
                print(f"   ✅ Cargado desde checkpoint (formato directo)")
        
        if verbose:
            print(f"   ✅ Transfer learning completado")
            print(f"      - Misma arquitectura: Transferencia completa")
...
def main():
    ...
    print("=" * 70)
    print("\n⚠️  Este entorno es el MÁS DIFÍCIL - requiere GENERALIZACIÓN")
    print("   Los objetos aparecen en posiciones ALEATORIAS cada episodio")
    ...
    model_path = os.path.join(models_dir, 'entorno3_mejor.pth')
    pretrained_path = os.path.join(models_dir, 'entorno2_mejor.pth')
    ...
    print(f"   - Crucial para generalización")
    
    # Crear agente DQN optimizado para generalización
    agent = DQNAgent(
        state_size=state_size,
        action_size=action_size,
        feedback=feedback,
        learning_rate=0.0003,      # LR bajo para estabilidad
        gamma=0.99,
        epsilon=0.3,               # Empezar con epsilon bajo (transfer learning)
        epsilon_min=0.05,          # Epsilon mínimo más alto para exploración continua
        epsilon_decay=0.998,       # Decay lento
    ...
    print(f"\n✅ Agente DQN creado (optimizado para generalización)")
    print(f"   - Learning rate: 0.0003 (bajo para estabilidad)")
    print(f"   - Gamma: 0.99")
    print(f"   - Epsilon: 0.3 → 0.05 (decay: 0.998)")
    print(f"   - Buffer size: 30,000 (grande para diversidad)")
    print(f"   - Batch size: 64")
    print(f"   - Arquitectura: [128, 64]")
    
    # Transfer Learning desde Entorno 2
    load_pretrained_weights_full(agent, pretrained_path, verbose=True)
    
    # Entrenar
    num_episodes = 8000  # Máximo
    
    print("\n" + "=" * 70)
    print("INICIANDO ENTRENAMIENTO")
    print("=" * 70)
    print("NOTA: El modelo se guarda SOLO cuando mejora el SUCCESS RATE")
    print("NOTA: Este entorno puede requerir MÁS episodios para generalizar")
    print("=" * 70)
    ...
    agent.train(
        ...
        verbose=True,
        early_stopping=True, 
        target_success_rate=0.90,  # 90% objetivo (margen sobre el 85% requerido)
        patience=1500,
        eval_freq=200,
    ...
    fig.suptitle('Entorno 3: Objetos Aleatorios, Recogida y Entrega (Generalización)', 
                 fontsize=14, fontweight='bold')
    
    # Recompensas
    axes[0, 0].plot(agent.training_rewards, alpha=0.3, color='blue')
    window = 200  # Ventana más grande para suavizar ruido
    ...
    if agent.success_rates_history:
        ...
        axes[1, 0].axhline(y=85, color='r', linestyle='--', label='Objetivo (85%)')
    ...
    Modelo guardado: entorno3_mejor.pth
    Transfer Learning: Desde entorno2_mejor.pth
    ...
    Objetivo: ≥ 85%
    Estado: {'✅ CUMPLIDO' if results['success_rate'] >= 85 else '❌ NO CUMPLIDO'}
    
    NOTA: Este entorno requiere GENERALIZACIÓN
    """
    ...
    plt.tight_layout()
    graph_path = os.path.join(outputs_dir, 'entorno3_training.png')
