...
class WarehouseEnv(gym.Env):
    ...
    def __init__(
        ...
        self.r_step = -0.02
        self.r_collision = -15.0
        self.r_pick = 100.0
        self.r_delivery = 300.0
        self.r_wrong_drop = -20.0
    ...
    def step(self, action):
        self.steps += 1
        reward = self.r_step  # Pequeña penalización por cada paso
        terminated = False
        truncated = False
        ...
        if action < 4:  # Movement (0: Up, 1: Down, 2: Left, 3: Right)
            ...
            if not self._is_collision(new_pos):
                ...
                # SHAPED REWARD: Recompensa por acercarse al objetivo
                if not self.agent_has_object:
                    # Sin objeto: acercarse a objetos
                    current_distance = self._get_distance_to_nearest_object()
                    # ✅ CORREGIDO: Solo dar reward si hay objetos disponibles
                    if (
                        current_distance != float("inf")
                        and current_distance < self.previous_distance_to_nearest_object
                    ):
                        reward += 0.05  # ✅ Aumentado de 0.1 a 0.5
                    # ✅ Actualizar solo si hay objetos
                    if current_distance != float("inf"):
                        self.previous_distance_to_nearest_object = current_distance
                else:
                    # Con objeto: acercarse a área de entrega
                    delivery_center = (
                        self.delivery_area[0] + self.delivery_area[2] / 2,
                        self.delivery_area[1] + self.delivery_area[3] / 2,
                    )
                    ...
                    if current_distance < self.previous_distance_to_delivery:
                        reward += 0.5  # ✅ Aumentado de 0.2 a 1.0
            ...
            if not self.agent_has_object:
                ...
                if not picked:
                    # ✅ Aumentar penalización por intentar recoger lejos
                    reward = -2.0  # Era -1.0
            ...
            if self.agent_has_object:
                if self._is_in_area(self.agent_pos, self.delivery_area):
                    ...
                    # ENTREGA FALLIDA (fuera del área)
                    reward = self.r_wrong_drop
                    self.object_positions.append(self.agent_pos)
                    terminated = True
                self.agent_has_object = False
            else:
                # ✅ Aumentar penalización por intentar soltar sin objeto
                reward = -2.0  # Era -1.0

        # Truncamiento por timeout
        if self.steps >= self.max_steps:
            truncated = True
            reward += -10.0  # ✅ Aumentado de -5.0 (penalizar más el timeout)
    ...
    def _get_obs(self):
        ...
        for i in range(3):
            if i < len(self.object_positions) and self.object_positions[i] is not None:
                ...
                obs[2 + 2 * i : 4 + 2 * i] = [10, 10]  # Fuera del mapa
